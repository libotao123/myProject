...
@JobHandler(value="update_ota_proname")
@Component
/*
 * 更新ota版本产品型号
 * 
 * 
 */
public class UpdateOtaPronameJobHandler extends IJobHandler {
	protected final static Logger logger = LoggerFactory.getLogger(UpdateOtaPronameJobHandler.class);
	...
	public int queryOtaDeviceCount(){
		String sql = "SELECT COUNT(*) FROM cc_ota WHERE org_code LIKE '%-%' ";
		return jdbcTemplate.queryForObject(sql, Integer.class);
	}
    	
	public List<Map<String,Object>> queryPage(int start,int end){
		String sql = "SELECT ota_id,org_code FROM cc_ota WHERE org_code LIKE '%-%' ORDER BY ota_id LIMIT ?,? ";
		return jdbcTemplate.queryForList(sql, start,end);
	}
	
	public int[] updateProname(List<Map<String,Object>> list){
		String sql = "UPDATE cc_ota SET pro_name = ? WHERE ota_id = ? ";
		...
		jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
			...
			public void setValues(PreparedStatement ps, int i) throws SQLException {
				...
				ps.setString(2, list.get(i).get("ota_id").toString());
				i++;
			}

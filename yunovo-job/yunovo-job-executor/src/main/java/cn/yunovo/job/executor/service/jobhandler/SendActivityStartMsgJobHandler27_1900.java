...
/*
 * 定时推送模板消息27_0930（永盛杰卡）
 * 
 * 
 */
public class SendActivityStartMsgJobHandler27_1900 extends IJobHandler {
	protected final static Logger logger = LoggerFactory.getLogger(SendActivityStartMsgJobHandler27_1900.class);
...
//            logger.error("[SendTemplateMessageJobHandler.sendMsgHandler][发送模板消息异常],e={}",e1);
            XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][新增数据异常],e={0}",ExceptionUtils.getStackTrace(e1));
            return a;
		}
    ...
    public void sendMsgHandler() throws Exception {
    	...
    	XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][A类用户总数],sumA={0}",yds_result.size());
    	try {
			for (int i=0; i<yds_result.size(); i++) {
				JSONObject by_wx = yds_result.getJSONObject(i);
				String open_id = by_wx.getString("open_id");
				try {
                  	sendTemplateMessage("oXEDlt_p6Tzeqd8Yo0DyE1v-9IMg",appid, appsecret, toTemMesJson1("oXEDlt_p6Tzeqd8Yo0DyE1v-9IMg",HOST+WX_LINK_URL+"userA"));
                  	break;
					//result += sendTemplateMessage(open_id,appid, appsecret, toTemMesJson1(open_id,HOST+WX_LINK_URL+"userA&openId="+open_id));
				} catch (Exception e) {
					XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][A类用户发送模板消息失败],open_id={0},e={1}",open_id,ExceptionUtils.getStackTrace(e));
				}
			}
			XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][A类用户发送成功数],successA={0}",result);
		} catch (Exception e) {
			XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][A类用户发送模板消息异常],e={0}",ExceptionUtils.getStackTrace(e));
		}
    	
    	//给B类用户发消息
    	result = 0;
    	yds_result = findYundashData(query_id2,yundash_api_key);
    	XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][B类用户总数],sumB={0}",yds_result.size());
    	try {
			for (int i=0; i<yds_result.size(); i++) {
				JSONObject by_wx = yds_result.getJSONObject(i);
				String open_id = by_wx.getString("open_id");
				try {
                  	sendTemplateMessage("oXEDlt_p6Tzeqd8Yo0DyE1v-9IMg",appid, appsecret, toTemMesJson2("oXEDlt_p6Tzeqd8Yo0DyE1v-9IMg",HOST+WX_LINK_URL+"userA"));
                  	break;
					//result += sendTemplateMessage(open_id,appid, appsecret, toTemMesJson2(open_id,HOST+WX_LINK_URL+"userB&openId="+open_id));
				} catch (Exception e) {
					XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][B类用户发送模板消息失败],open_id={0},e={1}",open_id,ExceptionUtils.getStackTrace(e));
				}
			}
			XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][B类用户发送成功数],successB={0}",result);
		} catch (Exception e) {
            XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][B类用户发送模板消息异常],e={0}",ExceptionUtils.getStackTrace(e));
		}
    	
    	//给C类用户发消息
    	result = 0;
    	yds_result = findYundashData(query_id3,yundash_api_key);
    	XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][C类用户总数],sumC={0}",yds_result.size());
    	try {
			for (int i=0; i<yds_result.size(); i++) {
				JSONObject by_wx = yds_result.getJSONObject(i);
				String open_id = by_wx.getString("open_id");
				try {
                  	sendTemplateMessage("oXEDlt_p6Tzeqd8Yo0DyE1v-9IMg",appid, appsecret, toTemMesJson3("oXEDlt_p6Tzeqd8Yo0DyE1v-9IMg",HOST+WX_LINK_URL+"userA"));
                  	break;
					//result += sendTemplateMessage(open_id,appid, appsecret, toTemMesJson3(open_id,HOST+WX_LINK_URL+"userC&openId="+open_id));
				} catch (Exception e) {
					XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][C类用户发送模板消息失败],open_id={0},e={1}",open_id,ExceptionUtils.getStackTrace(e));
				}
			}
			XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][C类用户发送成功数],successC={0}",result);
		} catch (Exception e) {
            XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendMsgHandler][C类用户发送模板消息异常],e={0}",ExceptionUtils.getStackTrace(e));
		}
    }
...
//		logger.info("[SendTemplateMessageJobHandler][开始调yundash查询用户]  url={}",yds_query_url);
		XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900][开始调yundash查询用户]  url={0}",yds_query_url);
	...
	public int sendTemplateMessage(String open_id,String appid, String appsecret,String message) {
		...
		if(StringUtils.equals(jsonObject.getString("errcode"),"0")){
			logger.info("[SendActivityStartMsgJobHandler27_1900.sendTemplateMessage][发送微信模板消息成功]open_id={},result={}",open_id,result);
            XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendTemplateMessage][发送微信模板消息成功]open_id={0},result={1}",open_id,result);
			return 1;
		}else {
			logger.info("[SendActivityStartMsgJobHandler27_1900.sendTemplateMessage][发送微信模板消息失败]open_id={},result={},message={},url={}",open_id,result,message,url);
            XxlJobLogger.log("[SendActivityStartMsgJobHandler27_1900.sendTemplateMessage][发送微信模板消息失败]open_id={0},result={1},message={2},url={3}",open_id,result,message,url);
			return 0;
		}
	...
	public String getAccessToken(String appid, String appsecret, String cleanCache) {
		...
		if(!json.containsKey("access_token") || "".equals(json.getString("access_token"))) {
			logger.info("[SendActivityStartMsgJobHandler27_1900.getAccessToken][ERROR]从网络获取access_token失败,result={},params={}",result,JSONObject.toJSONString(params));
		}else {
			logger.info("[SendActivityStartMsgJobHandler27_1900.getAccessToken]从网络获取到access_token成功，access_token={},params={}",json.getString("access_token"),JSONObject.toJSONString(params));
		}

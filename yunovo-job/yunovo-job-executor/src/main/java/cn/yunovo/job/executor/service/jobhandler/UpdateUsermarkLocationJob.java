...
@JobHandler(value="usermark_location_job")
@Component
public class UpdateUsermarkLocationJob extends IJobHandler {
...
//	String getLastPoint_url = "http://lsapi.prd.yunovo.cn/rest/api/trace/getLastPointByDid?did=%s";
	...
	public void main() throws Exception {
		...
		for(int i = 0 ; i < total; i++) {
			...
			if(obj != null && obj.size() > 0) {
				success_number++;
				this.update(device_sn, obj.getString("province"), obj.getString("city"), obj.getString("district"), obj.getString("location"), 1);
				continue;
			}
	...
	public JSONObject getLocationByTraces(String device_sn, long load_time) throws Exception {
		...
		if(arr2 != null && arr2.size() > 0) {
			...
			/*if(Math.abs(start_at2-load_time) > diff_time) {
				XxlJobLogger.log("[UpdateUsermarkLocationJob.getLocationByTraces]轨迹时间2与装车时间差值大于12H，device_sn={0},load_time={1},start_at2={2}",device_sn,load_time,start_at2);
				start_at2 = -1;
			}*/
		}
	...
	public JSONObject getLocationByDriving(String device_sn, long load_time) throws Exception {
		...
		if(arr2 != null && arr2.size() > 0) {
			...
			//判断轨迹时间是否与装车时间在12H内
			/*if(Math.abs(start_at2-load_time) > diff_time) {
				XxlJobLogger.log("[UpdateUsermarkLocationJob.getLocationByDriving]轨迹时间2与装车时间差值大于12H，device_sn={0},load_time={1},start_at2={2}",device_sn,load_time,start_at2);
				start_at2 = -1;
			}*/
		}
...
/*	//update location_type
	...
	public List<Map<String,Object>> queryUsermark() {
		String sql = "SELECT device_sn,time_added FROM cc_device_event_usermark WHERE created_time < '2020-09-16 00:00:00' and ((province is null and city is null and district is null and location is null) or (province = '' and city = '' and district = '' and location = '')) ";
		return jdbcTemplate.queryForList(sql);
		
	}
	
	public int update(String device_sn,String province,String city,String district,String location,int location_type) {
		String sql = "UPDATE cc_device_event_usermark SET location_type = ?, province = ?,city = ?,district = ?,location = ? WHERE device_sn = ? ";
		return jdbcTemplate.update(sql, location_type, province, city, district, location, device_sn);
	}

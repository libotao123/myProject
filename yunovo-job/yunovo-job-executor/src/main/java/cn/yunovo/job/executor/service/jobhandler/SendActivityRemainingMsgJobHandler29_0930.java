 ...
 * 
 */
public class SendActivityRemainingMsgJobHandler29_0930 extends IJobHandler {
	protected final static Logger logger = LoggerFactory.getLogger(SendActivityRemainingMsgJobHandler29_0930.class);
...
//            logger.error("[SendTemplateMessageJobHandler.sendMsgHandler][发送模板消息异常],e={}",e1);
            XxlJobLogger.log("[SendActivityRemainingMsgJobHandler29_0930.sendMsgHandler][新增数据异常],e={0}",ExceptionUtils.getStackTrace(e1));
            return a;
		}
    ...
    public void sendMsgHandler() throws Exception {
    	...
    	XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][A类用户总数],sumA={0}",yds_result.size());
    	try {
			for (int i=0; i<yds_result.size(); i++) {
				...
				try {
					...
					XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][A类用户发送模板消息失败],open_id={0},e={1}",open_id,ExceptionUtils.getStackTrace(e));
				}
			}
			XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][A类用户发送成功数],successA={0}",result);
		} catch (Exception e) {
			XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][A类用户发送模板消息异常],e={0}",ExceptionUtils.getStackTrace(e));
		}
    	
    	//给B类用户发消息
    	result = 0;
    	yds_result = findYundashData(query_id2,yundash_api_key);
    	XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][B类用户总数],sumB={0}",yds_result.size());
    	try {
			for (int i=0; i<yds_result.size(); i++) {
				JSONObject by_wx = yds_result.getJSONObject(i);
				String open_id = by_wx.getString("open_id");
				try {
					...
					XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][B类用户发送模板消息失败],open_id={0},e={1}",open_id,ExceptionUtils.getStackTrace(e));
				}
			}
			XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][B类用户发送成功数],successB={0}",result);
		} catch (Exception e) {
			XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][B类用户发送模板消息异常],e={0}",ExceptionUtils.getStackTrace(e));
		}
    	
    	//给C类用户发消息
    	result = 0;
    	yds_result = findYundashData(query_id3,yundash_api_key);
    	XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][C类用户总数],sumC={0}",yds_result.size());
    	try {
			for (int i=0; i<yds_result.size(); i++) {
				JSONObject by_wx = yds_result.getJSONObject(i);
				String open_id = by_wx.getString("open_id");
				try {
					...
					XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][C类用户发送模板消息失败],open_id={0},e={1}",open_id,ExceptionUtils.getStackTrace(e));
				}
			}
			XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][C类用户发送成功数],successC={0}",result);
		} catch (Exception e) {
			XxlJobLogger.log("[SendActivityStartMsgJobHandler29_0930.sendMsgHandler][C类用户发送模板消息异常],e={0}",ExceptionUtils.getStackTrace(e));
		}
    }
...
//		logger.info("[SendTemplateMessageJobHandler][开始调yundash查询用户]  url={}",yds_query_url);
	...
	public String getAccessToken(String appid, String appsecret, String cleanCache) {
		...
		if(!json.containsKey("access_token") || "".equals(json.getString("access_token"))) {
			...
			XxlJobLogger.log("[SendActivityRemainingMsgJobHandler29_0930.getAccessToken]从网络获取access_token失败,result={0},params={1}",result,JSONObject.toJSONString(params));
		}else {
			...
			XxlJobLogger.log("[SendActivityRemainingMsgJobHandler29_0930.getAccessToken]从网络获取access_token成功,access_token={0},params={1}",json.getString("access_token"),JSONObject.toJSONString(params));
		}
	...
	public String toTemMesJson1 (String openid, String url) {
		...
		data.put("first", toVC_JsonObj("#周年聚惠#最后一天！\n充值立返最高88元现金红包！返现名额不足100人，手慢无！\n","#FF2400"));
	...
	public String toTemMesJson2 (String openid, String url) {
		...
		JSONObject data = new JSONObject();
		data.put("first", toVC_JsonObj("#周年聚惠#最后一天！\n充值立返最高88元现金红包！返现名额已不足100人，手慢无！网络音乐时时听，在线导航随处享！\n","#FF2400"));
		data.put("keyword1", toVC_JsonObj("3周年庆典！周年聚惠趴，好礼2连发！","#000001"));
		data.put("keyword2", toVC_JsonObj("最后一天","#000001"));
		data.put("keyword3", toVC_JsonObj("2019.7.27-2019.7.29","#000001"));
		data.put("remark", toVC_JsonObj("\n\n更有机会获得免单、油卡……\n戳我开抢☞☞☞","#2087D9"));
	...
	public String toTemMesJson3 (String openid, String url) {
		...
		JSONObject data = new JSONObject();
		data.put("first", toVC_JsonObj("#周年聚惠#最后一天！\n充值立返最高88元现金红包！返现名额已不足100人，手慢无！幸运转盘，转出好运，免单、油卡免费抽！\n","#FF2400"));
		data.put("keyword1", toVC_JsonObj("3周年庆典！周年聚惠趴，好礼2连发！","#000001"));
		data.put("keyword2", toVC_JsonObj("最后一天","#000001"));
		data.put("keyword3", toVC_JsonObj("2019.7.27-2019.7.29","#000001"));
		data.put("remark", toVC_JsonObj("\n\n更有机会获得免单、油卡……\n戳我开抢☞☞☞","#2087D9"));

$(function() {
	...
	$("#jobGroup").on("change", function () {
		...
		$.ajax({
			...
			url : base_url + '/secure/joblog/getJobsByGroup',
	...
	var logTable = $("#joblog_list").dataTable({
	    ...
	    "serverSide": true,
		"ajax": {
	        url: base_url + "/secure/joblog/pageList" ,
            type:"post",
	        data : function ( d ) {
	        	var obj = {};
	...
	$('#joblog_list').on('click', '.logDetail', function(){
		...
		window.open(base_url + '/secure/joblog/logDetailPage?id=' + _id);
		return;
	});
	...
	$('#joblog_list').on('click', '.logKill', function(){
        ...
        layer.confirm( (I18n.system_ok + I18n.joblog_kill_log + '?'), {
            ...
            $.ajax({
                type : 'POST',
                url : base_url + '/secure/joblog/logKill',
                data : {"id":_id},
                dataType : "json",
                success : function(data){
                    if (data.code == 200) {
	...
	$("#clearLogModal .ok").on('click', function(){
		$.post(base_url + "/secure/joblog/clearLog",  $("#clearLogModal .form").serialize(), function(data, status) {
			if (data.code == "200") {
